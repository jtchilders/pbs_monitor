...
Updated 2026-06-10 to flow through the multi-system repository API:
fixtures now stamp ``config.pbs.system = 'polaris'`` so writes have a
system to attach to. The tests themselves don't pass ``system``
explicitly — they're exercising the daemon-style write path, which
auto-stamps from config.
"""
...
def temp_db_config(tmp_path):
    ...
    # picked up by accident. Stamp pbs.system so write paths in the
    # repositories have a value to attach.
    config = Config(config_file=str(tmp_path / "none.yaml"))
    config.pbs.system = "polaris"
